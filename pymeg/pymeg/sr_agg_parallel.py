...
def aggregate(subject, session, datatype):
    ...
    if datatype == 'F':    # time-frequency
        agg = asr.aggregate_files(data, data, (-0.4, -0.2), to_decibels=True, all_clusters=clusters, hemis=['Single'])
    elif datatype == 'BB':    # broadband
        agg = asr.aggregate_files(data, data, (-0.2, 0), to_decibels=False, all_clusters=clusters, hemis=['Single'])
